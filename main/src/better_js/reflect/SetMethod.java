...
public class SetMethod {
	...
	private static Object generateMethod(Method m) {
		...
		newMethod.setFuncSelf("invoke", cfw -> {
			...
			if (!isStatic) {
				cfw.addALoad(1);
				cfw.add(ByteCode.CHECKCAST, dot2slash(base));
			}
			...
			cfw.addInvoke(Modifier.isInterface(m.getModifiers()) ? ByteCode.INVOKEINTERFACE
					...
					dot2slash(base), m.getName(), nativeMethod(m.getReturnType(), args));
	...
	public static void init_dis2(MyClass<?> myFactoryClass) {
		myFactoryClass.setFunc("newMethodAccessor", (self, _args) -> {
			...
			if (false) return classObjectMapObjectMap.get(m, () -> {
				...
				cfw.add(ByteCode.GETSTATIC, myClass2.adapterName, "__handle__", LtypeName(MethodHandle.class));
				if (!isStatic) {
					cfw.add(ByteCode.ALOAD, 1);
					cfw.add(ByteCode.CHECKCAST, dot2slash(m.getDeclaringClass()));
				}
				int i = 0;
				for (; i < pars.length; i++) {
				...
				String ObjectName = LtypeName(Object.class);
				cfw.addInvoke(ByteCode.INVOKEVIRTUAL, dot2slash(MethodHandle.class), "invoke", "(" + (isStatic ? "" : LtypeName(m.getDeclaringClass())) + ObjectName.repeat(pars.length) + ")" + ObjectName);
				cfw.add(ByteCode.ARETURN);
