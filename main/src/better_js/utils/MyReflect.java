...
import better_js.android.DxLoader.*;
...
import static better_js.utils.MyReflectAndroid.*;
...
public class MyReflect {
	...
	public static void setOverride(AccessibleObject obj) {
		obj.setAccessible(true);
	}
	public static ClassLoader loader;
	public static Class<?> defineClassNative(String name, Class<?> superClass, byte[] bytes) {
		if (OS.isAndroid) {
			...
			try {
				return AndroidDefiner.defineClassNative(name, bytes);
			} catch (Exception e) {
				Log.err(e);
				return null;
			}
		} else {
			return JDKVars.junsafe.defineClass0(name, bytes, 0, bytes.length, null, null);
		}
	}

	public static Class<?> defineCustomClass(String name, Class<?> superClass, byte[] bytes) {
		if (OS.isAndroid) {
			int mod = superClass.getModifiers();
			if (/*Modifier.isFinal(mod) || */!Modifier.isPublic(mod)) {
				setPublic(superClass);
			...
			try {
				...
				Log.err("Unexpected Throwable", e);
	...
	public static void setValue(Object obj, Field f, Object value, boolean force) {
		...
		long offset = getOffset(f, isStatic);
		if (force) {
	...
	public static long getOffset(Field f, boolean isStatic) {
		return OS.isAndroid ? FieldUtils.getFieldOffset(f)
	...
	public static OffsetProvider provider;

	static {
		try {
			/* 判断是否能获取 */
			JDKVars.junsafe.getClass();
			provider = JDKVars.junsafe::objectFieldOffset;
		} catch (Throwable th) {
			provider = ((cl, name) -> {
				try {
					return unsafe.objectFieldOffset(cl.getDeclaredField(name));
				} catch (NoSuchFieldException e) {throw new RuntimeException(e);}
			});
		}
	}

	public static long offset(Class<?> cl, String name) {
		return provider.get(cl, name);
	}
	...
	public static void setValue(Class<?> cls, String name, Object value) throws NoSuchFieldException {
		setValue(cls, cls.getDeclaredField(name), value, false);
	}
