...
public class ScriptRuntime {
	...
	public static final Class<?> ContextClass = Kit.classOrNull("rhino.Context"),
	 ContextFactoryClass                      = Kit.classOrNull("rhino.ContextFactory"),
	 FunctionClass                            = Kit.classOrNull("rhino.Function"),
	 ScriptableObjectClass                    = Kit.classOrNull("rhino.ScriptableObject");
	...
	public static ScriptableObject initSafeStandardObjects(
	 Context cx, ScriptableObject scope, boolean sealed) {
		...
		new LazilyLoadedCtor(
		 scope, "RegExp", "rhino.regexp.NativeRegExp", sealed, true);
		new LazilyLoadedCtor(
		 scope, "Continuation", "rhino.NativeContinuation", sealed, true);
	...
	public static ScriptableObject initStandardObjects(
	 Context cx, ScriptableObject scope, boolean sealed) {
		...
		new LazilyLoadedCtor(
		 s, "Packages", "rhino.NativeJavaTopPackage", sealed, true);
		new LazilyLoadedCtor(
		 s, "getClass", "rhino.NativeJavaTopPackage", sealed, true);
		new LazilyLoadedCtor(s, "JavaAdapter", "rhino.JavaAdapter", sealed, true);
		new LazilyLoadedCtor(
		 s, "JavaImporter", "rhino.ImporterTopLevel", sealed, true);

		for (String packageName : getTopPackageNames()) {
			new LazilyLoadedCtor(
			 s, packageName, "rhino.NativeJavaTopPackage", sealed, true);
		}
	...
	public static Object evalSpecial(
	 ...
	 int lineNumber) {
		...
		// mode.
		Script script = cx.compileString(x.toString(), evaluator, reporter, sourceName, 1);
	...
	public static ScriptableObject getGlobal(Context cx) {
		final String GLOBAL_CLASS = "rhino.tools.shell.Global";
	...
	private static class DefaultMessageProvider implements MessageProvider {
		...
		public String getMessage(String messageId, Object[] arguments) {
			final String defaultResource = "rhino.resources.Messages";
