...
import static rhino.Context.reportRuntimeError;
...
public class MyNativeJavaMethod extends BaseFunction {
	MyNativeJavaMethod(MyMemberBox[] methods) {
		this.functionName = methods[0].getName();
		this.methods = methods;
	}

	MyNativeJavaMethod(MyMemberBox[] methods, String name) {
		this.functionName = name;
		this.methods = methods;
	}

	MyNativeJavaMethod(MyMemberBox method, String name) {
		this.functionName = name;
		this.methods = new MyMemberBox[]{method};
	}

	public MyNativeJavaMethod(Method method, String name) {
		this(new MyMemberBox(method), name);
	}
	...
	static String scriptSignature(Object[] values) {
		...
		for (int i = 0; i != values.length; ++i) {
			...
			if (value == null) {
				...
				s = MyJavaMembers.javaSignature(value.getClass());
			}
	...
	public String toString() {
		...
		for (int i = 0, N = methods.length; i != N; ++i) {
			...
			if (methods[i].isMethod()) {
				...
				sb.append(MyJavaMembers.javaSignature(method.getReturnType()));
				sb.append(' ');
				sb.append(method.getName());
			} else {
				sb.append(methods[i].getName());
			}
			sb.append(MyJavaMembers.liveConnectSignature(methods[i].argTypes));
			sb.append('\n');
		}
		return sb.toString();
	}
		...
		for (int i = 0; i < methodsOrCtors.length; i++) {
			...
			if (firstBestFit < 0) {
				...
				for (int j = -1; j != extraBestFitsCount; ++j) {
					...
					MyMemberBox bestFit = methodsOrCtors[bestFitIndex];
	...
	private static int preferSignature(Object[] args,
		...
		for (int j = 0; j < args.length; j++) {
			...
			int rank1 = MyNativeJavaObject.getConversionWeight(arg, type1);
			int rank2 = MyNativeJavaObject.getConversionWeight(arg, type2);
			...
			if (rank1 < rank2) {
				...
				// Equal ranks
				if (rank1 == MyNativeJavaObject.CONVERSION_NONTRIVIAL) {
	...
	private static void printDebug(String msg, MyMemberBox member,
		...
		if (debug) {
			...
			sb.append(MyJavaMembers.liveConnectSignature(member.argTypes));
